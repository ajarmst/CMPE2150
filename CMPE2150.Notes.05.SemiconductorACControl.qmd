...
higher voltage than the Emitter, and control is exercised by running a current through the positively-biased Base to Emitter junction. For aNP transistor, the Collector must be at a lower voltage than the
...
only one of these---the Silicon Controlled Regulator(SCR). Because of this uncertainty in
...
The basic four-layer device is called the Shockley Diode, although it is not actually a diode (the names of these devices can be a bit confusing, as you will see)[^1]. 
...
The Shockley Diode has only two pins, and is given the symbol in the margin.

::: {.column-margin}
![](images/ShockleySymbol.jpg){size=50%}
...
VAK then drops to a much lower value, $V_F$, as it has now been turned On.
:::
